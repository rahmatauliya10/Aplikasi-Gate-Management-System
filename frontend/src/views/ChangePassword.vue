    ...
    <div class="login-center">
      ...
      <div class="login-card" :class="{ 'shake': shakeCard, 'card-loaded': mounted }">
        <div class="card-header">
          ...
          <p class="card-subtitle">Wajib mengganti temporary password Anda</p>
        </div>
        ...
        <form class="login-form" @submit.prevent="handleChangePassword" id="change-password-form">
          <div class="field" :class="{ 'field-active': focusCurrent || formData.currentPassword }">
            ...
            <div class="field-input-wrap">
              <label for="input-current" class="field-label">Password Saat Ini</label>
              <input
                id="input-current"
          ...
          <div class="field" :class="{ 'field-active': focusNew || formData.newPassword }">
            <div class="field-icon">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <rect x="3" y="11" width="18" height="11" rx="2" ry="2"/>
                <path d="M7 11V7a5 5 0 0 1 10 0v4"/>
            ...
            <div class="field-input-wrap">
              <label for="input-new" class="field-label">Password Baru</label>
              <input
                id="input-new"
          ...
          <div class="field" :class="{ 'field-active': focusConfirm || formData.confirmPassword }">
            <div class="field-icon">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <rect x="3" y="11" width="18" height="11" rx="2" ry="2"/>
                <path d="M7 11V7a5 5 0 0 1 10 0v4"/>
            ...
            <div class="field-input-wrap">
              <label for="input-confirm" class="field-label">Konfirmasi Password Baru</label>
              <input
                id="input-confirm"
...
const handleChangePassword = async () => {
  if (formData.newPassword.length < 8) {
    statusMsg.value = 'Password baru minimal 8 karakter'
  ...
  // Custom validation rule from DTO: at least one letter and one number
  if (!/^(?=.*[A-Za-z])(?=.*\d).*$/.test(formData.newPassword)) {
    statusMsg.value = 'Password harus mengandung huruf dan angka'
    statusClass.value = 'status-error'
    statusIcon.value = '❌'
    shakeCard.value = true
    setTimeout(() => { shakeCard.value = false }, 600)
  ...
  if (formData.newPassword !== formData.confirmPassword) {
    statusMsg.value = 'Password baru dan konfirmasi tidak cocok'
    statusClass.value = 'status-error'
    statusIcon.value = '❌'
    shakeCard.value = true
    setTimeout(() => { shakeCard.value = false }, 600)
  ...
  try {
    ...
    const errMsg = err.gmsMessage || err.response?.data?.message || 'Gagal mengubah password'
...
.login-center { position: relative; z-index: 10; width: 100%; max-width: 420px; padding: 24px 20px; display: flex; flex-direction: column; align-items: center; }
.logo-section { display: flex; flex-direction: column; align-items: center; margin-bottom: 28px; opacity: 0; transform: translateY(-20px); transition: all 0.8s cubic-bezier(0.16, 1, 0.3, 1); position: relative; }
...
.login-logo { width: 72px; height: 72px; object-fit: contain; filter: drop-shadow(0 4px 20px rgba(59, 130, 246, 0.4)); animation: logoFloat 4s ease-in-out infinite; margin-bottom: 12px; position: relative; z-index: 1; }
...
.login-card { width: 100%; background: rgba(255, 255, 255, 0.07); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); border: 1px solid rgba(255, 255, 255, 0.12); border-radius: 20px; padding: 32px 28px; box-shadow: 0 24px 64px rgba(0, 0, 0, 0.3), 0 0 0 1px rgba(255, 255, 255, 0.05) inset, 0 1px 0 rgba(255, 255, 255, 0.1) inset; opacity: 0; transform: translateY(24px) scale(0.97); transition: all 0.7s cubic-bezier(0.16, 1, 0.3, 1) 0.2s; }
...
.card-subtitle { font-size: 13px; color: rgba(148, 197, 255, 0.6); margin: 0; font-weight: 400; }
.status-bar { display: flex; align-items: center; gap: 8px; padding: 10px 14px; border-radius: 10px; font-size: 13px; font-weight: 600; margin-bottom: 16px; animation: slideDown 0.3s ease both; }
...
.field { position: relative; display: flex; align-items: center; gap: 10px; padding: 10px 14px; border-radius: 12px; background: rgba(255, 255, 255, 0.04); border: 1.5px solid rgba(255, 255, 255, 0.08); transition: all 0.3s ease; }
.field:hover { background: rgba(255, 255, 255, 0.06); border-color: rgba(255, 255, 255, 0.12); }
.field.field-active { background: rgba(59, 130, 246, 0.08); border-color: rgba(59, 130, 246, 0.4); box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.08); }
.field-icon { color: rgba(148, 197, 255, 0.5); flex-shrink: 0; transition: color 0.3s; display: flex; }
.field.field-active .field-icon { color: #60a5fa; }
.field-input-wrap { flex: 1; position: relative; }
.field-label { position: absolute; top: 50%; left: 0; transform: translateY(-50%); font-size: 14px; color: rgba(148, 197, 255, 0.4); pointer-events: none; transition: all 0.25s ease; font-weight: 500; }
.field.field-active .field-label { top: -2px; transform: translateY(-100%); font-size: 10px; color: #60a5fa; font-weight: 600; letter-spacing: 0.04em; }
.field-input-wrap input { width: 100%; height: 28px; border: none; background: transparent; color: #ffffff; font-size: 14px; font-family: 'Inter', sans-serif; font-weight: 500; outline: none; }
.field-input-wrap input::-ms-reveal, .field-input-wrap input::-ms-clear { display: none !important; }
.field-input-wrap input::placeholder { color: transparent; }
.field-line { position: absolute; bottom: 0; left: 14px; right: 14px; height: 2px; background: linear-gradient(90deg, transparent, #3b82f6, transparent); border-radius: 1px; opacity: 0; transform: scaleX(0); transition: all 0.35s ease; }
.field.field-active .field-line { opacity: 1; transform: scaleX(1); }
.eye-btn { background: none; border: none; color: rgba(148, 197, 255, 0.4); cursor: pointer; padding: 2px; display: flex; transition: color 0.2s; }
.eye-btn:hover { color: #60a5fa; }
.btn-login { width: 100%; height: 46px; border: none; border-radius: 12px; font-family: 'Inter', sans-serif; font-size: 15px; font-weight: 700; cursor: pointer; color: white; background: linear-gradient(135deg, #2563eb 0%, #3b82f6 50%, #0ea5e9 100%); box-shadow: 0 4px 20px rgba(37, 99, 235, 0.4); transition: all 0.3s ease; margin-top: 4px; position: relative; overflow: hidden; }
.btn-login::before { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, transparent 0%, rgba(255,255,255,0.15) 50%, transparent 100%); transform: translateX(-100%); transition: transform 0.5s ease; }
...
.btn-loader { display: flex; align-items: center; justify-content: center; gap: 8px; }
