...
async function main() {
  ...
  try {
    ...
    console.log('\n[2/5] Mengambil snapshot data awal dari seluruh 14 tabel database & attachment files...');
    const [
      users,
      userWarehouseAccess,
      transactions,
    ...
    console.log(`- Snapshot awal berhasil diambil (${totalRecordsBefore} total record dari 14 tabel).`);
    ...
    console.log('\n[4/5] Memulihkan seluruh data 14 tabel dari snapshot (Full Atomic Restore)...');
    await prisma.$transaction(async (tx) => {
    ...
    console.log('\n[5/5] Memverifikasi integritas data pasca-pemulihan untuk seluruh 14 tabel...');
    ...
    const restoreLog = {
      ...
      status: 'PASSED',
      details: `Full DR Restore Drill verified 100% record match across all 14 tables (${totalRecordsAfter} total records restored, ${uploadFilesCount} attachment files verified).`,
