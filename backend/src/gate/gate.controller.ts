...
export class GateController {
  ...
  @ApiResponse({ status: 201, description: 'Gate check-in created successfully' })
  @ApiResponse({ status: 400, description: 'Validation failed or active transaction exists' })
  checkIn(@Body() dto: CreateGateCheckInDto, @CurrentUser() user: JwtPayloadUser) {
  ...
  cancel(@Param('id') id: string, @Body() dto: CancelTransactionDto, @CurrentUser() user: JwtPayloadUser) {
