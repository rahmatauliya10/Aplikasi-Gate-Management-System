...
export class WeighbridgeService {
  ...
  async getQueue(query: WeighbridgeQueryDto, user: JwtPayloadUser) {
    const { page = 1, limit = 10, search, processType, type, startDate, endDate } = query;
  ...
  async submitWeighIn(transactionId: string, dto: WeighInDto, user: JwtPayloadUser) {
    this.logger.log(`Weigh-in attempt for transaction ${transactionId} by ${user.email}`);
    ...
    if (duplicateRecord) {
      this.logger.warn(`Duplicate weigh-in attempt rejected for transaction ${transactionId}`);
    ...
    this.logger.log(`Weigh-in successful: ${updated.transactionNumber}, weight: ${dto.weight}`);
  ...
  async submitWeighOut(transactionId: string, dto: WeighOutDto, user: JwtPayloadUser) {
    this.logger.log(`Weigh-out attempt for transaction ${transactionId} by ${user.email}`);
    ...
    if (duplicateRecord) {
      this.logger.warn(`Duplicate weigh-out attempt rejected for transaction ${transactionId}`);
    ...
    if (tx.processType === 'GBB' || tx.processType === 'GSP') {
      if (tx.grossWeight === null || tx.grossWeight === undefined) {
        throw new BadRequestException({
          success: false,
          message: 'Gross weight is not available for this transaction. Cannot calculate net weight.',
          errors: []
        });
      }
      ...
      if (tx.tareWeight === null || tx.tareWeight === undefined) {
        throw new BadRequestException({
          success: false,
          message: 'Tare weight is not available for this transaction. Cannot calculate net weight.',
          errors: []
        });
      }
    ...
    const isRejected = tx.status === 'INCOMING_CHECK_REJECTED' || tx.status === 'QC_VEHICLE_REJECTED';
    ...
    const updated = await this.prisma.$transaction(async (prismaTx) => {
      ...
      if (tx.actualWeight !== null && tx.actualWeight !== undefined) {
        ...
        const deviationPercent = (deviation / Math.max(netWeight, tx.actualWeight)) * 100;
        ...
        let fraudNotes = `Deviation: ${deviationPercent.toFixed(2)}% (${deviation} kg). Net Weight: ${netWeight} kg, Actual Weight: ${tx.actualWeight} kg.`;
        ...
        if (riskLevel !== 'SAFE') {
          await this.activityLogsService.logAction({
              ...
              referenceId: transactionId,
              description: JSON.stringify({ riskLevel, deviationPercent, deviation, netWeight, actualWeight: tx.actualWeight }),
        status: 'SUCCESS'
      }).catch(() => {});
        }
      }
    ...
    this.logger.log(`Weigh-out successful: ${updated.transactionNumber}, weight: ${dto.weight}, netWeight: ${netWeight}`);
  ...
  async getRecordDetail(transactionId: string, user: JwtPayloadUser) {
    ...
    return {
      ...
      data: {
        ...
        remarks: weighOutRecord?.remarks || weighInRecord?.remarks || tx.remarks || null
      }
    };
  }
}