...
export class WeighbridgeController {
  ...
  @ApiOperation({
    summary:
      'Get active weighbridge queue (IN/OUT) with pagination and filters',
  })
  @ApiResponse({ status: 200, description: 'Queue retrieved successfully' })
  getQueue(
    @Query() query: WeighbridgeQueryDto,
  ...
  @ApiResponse({
    status: 200,
    description: 'Weighbridge in completed successfully',
  })
  @ApiResponse({
    status: 400,
    description: 'Validation failed or transaction is not in REGISTERED status',
  })
  ...
  @ApiOperation({
    summary:
      'Process vehicle weigh-out (register second weight and compute net weight)',
  })
  @ApiResponse({
    status: 200,
    description: 'Weighbridge out completed successfully',
  })
  @ApiResponse({
    status: 400,
    description: 'Validation failed, wrong status, or weight calculation error',
  })
  @ApiResponse({ status: 404, description: 'Transaction not found' })
  weighOut(
    @Param('transactionId') transactionId: string,
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Get weighbridge record detail for a specific transaction',
  })
  @ApiResponse({
    status: 200,
    description: 'Weighbridge record retrieved successfully',
  })
  @ApiResponse({ status: 404, description: 'Transaction not found' })
  getRecord(
    @Param('transactionId') transactionId: string,
