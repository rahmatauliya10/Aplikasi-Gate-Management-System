...
export function getOrCreateBootstrapAdminPassword(secretFilePath?: string): string {
  const envPwd = (process.env.ADMIN_PASSWORD || process.env.DEFAULT_ADMIN_PASSWORD)?.trim();
  const weakPasswords = ['admin123', 'password', 'admin', 'supersecret', '12345678'];
  
  if (envPwd && envPwd.length >= 12 && !weakPasswords.includes(envPwd.toLowerCase())) {
    return envPwd;
  }

  const defaultPath = process.env.NODE_ENV === 'production'
    ? '/app/secrets/bootstrap_admin_password.txt'
    : path.resolve(process.cwd(), '../deploy/secrets/bootstrap_admin_password.txt');
