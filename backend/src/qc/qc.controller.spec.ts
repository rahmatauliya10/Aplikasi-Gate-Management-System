...
describe('QcController API Contract & Route Aliases', () => {
  ...
  it('should have route aliases configured for submitVehicleCheck', () => {
    const path = Reflect.getMetadata(PATH_METADATA, controller.submitVehicleCheck);
    expect(path).toEqual(['vehicle-result/:transactionId', 'vehicle-check/:transactionId']);
  });

  it('should have route aliases configured for submitIncomingCheck', () => {
    const path = Reflect.getMetadata(PATH_METADATA, controller.submitIncomingCheck);
    expect(path).toEqual(['incoming-result/:transactionId', 'incoming-check/:transactionId']);
  });
  ...
  it('should route submitVehicleCheck calls properly to QcService', async () => {
    ...
    mockQcService.submitVehicleCheck.mockResolvedValue({ status: 'QC_VEHICLE_PASSED' });

    const result = await controller.submitVehicleCheck('tx-123', dto, mockUser as any);
    expect(qcService.submitVehicleCheck).toHaveBeenCalledWith('tx-123', dto, 'user-qc-1', mockUser);
  ...
  it('should route submitIncomingCheck calls properly to QcService', async () => {
    const mockUser = { id: 'user-qc-1', role: 'QC', email: 'qc@test.com' };
    const dto = { result: 'PASS' } as any;
    mockQcService.submitIncomingCheck.mockResolvedValue({ status: 'INCOMING_CHECK_PASSED' });

    const result = await controller.submitIncomingCheck('tx-123', dto, mockUser as any);
    expect(qcService.submitIncomingCheck).toHaveBeenCalledWith('tx-123', dto, 'user-qc-1', mockUser);
