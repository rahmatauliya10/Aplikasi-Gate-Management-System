  ...
  <div class="flex flex-col relative w-full overflow-hidden">
    ...
    <div class="px-7 pt-7 pb-4">
      <div class="flex items-center justify-between relative bg-slate-100/80 rounded-full p-1.5 backdrop-blur-sm border border-white/50 shadow-[inset_0_2px_6px_rgba(0,0,0,0.02)]">
        ...
        <div v-for="step in steps" :key="step.id" 
             ...
             :class="currentStep === step.id ? 'bg-white text-indigo-600 shadow-[0_4px_16px_rgba(74,139,223,0.3)] h-10 px-5' : (currentStep > step.id ? 'bg-transparent text-[#4A8BDF] w-10 h-10' : 'bg-transparent text-slate-600 w-10 h-10 hover:bg-slate-200/50')"
    ...
    <div class="relative w-full min-h-[360px] grid">
      <transition :name="transitionName">
        ...
        <!-- STEP 1: TUJUAN -->
        <div v-if="currentStep === 1" class="col-start-1 row-start-1 w-full px-7 pt-2 pb-7" key="step1">
          <h3 class="text-sm font-black text-slate-800 mb-6 tracking-tight">Pilih Tujuan Proses</h3>
        ...
        <!-- STEP 2: KENDARAAN & MUATAN -->
        <div v-else-if="currentStep === 2" class="col-start-1 row-start-1 w-full px-7 pt-2 pb-7" key="step2">
          <h3 class="text-sm font-black text-slate-800 mb-4 tracking-tight">Data Kendaraan</h3>
          
          <div class="space-y-5">
            <!-- Vehicle Type Grid Selection -->
            <div>
              <label class="text-[10px] font-black text-slate-700 uppercase tracking-widest block mb-3">Tipe Kendaraan</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div class="floating-group">
                <input v-model="formattedPlate" type="text" class="floating-input font-mono uppercase" placeholder="Nomer Polisi" />
                <label class="floating-label">Nomer Polisi</label>
              </div>
            ...
            <div>
              <div class="flex items-center justify-between mb-2">
                <label class="text-[10px] font-black text-slate-700 uppercase tracking-widest">Daftar Muatan</label>
                <button type="button" @click.prevent="addCargoItem" class="text-[10px] font-bold text-[#4A8BDF] hover:text-indigo-700 bg-[#E6F0FA] hover:bg-indigo-100 px-3 py-1.5 rounded-lg flex items-center transition-colors">
                  <span class="material-icons text-[14px] mr-1">add_circle</span> Tambah
                </button>
              </div>
              <transition-group name="list" tag="div" class="space-y-2">
                <div v-for="(item, index) in form.cargoItems" :key="item.id" class="relative floating-group !pt-0">
                  <input :value="item.name" @input="item.name = $event.target.value.toUpperCase()" type="text" class="floating-input !pl-10 !pr-10 uppercase" placeholder="Nama Barang" />
        ...
        <!-- STEP 3: KEAMANAN -->
        <div v-else-if="currentStep === 3" class="col-start-1 row-start-1 w-full px-7 pt-2 pb-7" key="step3">
          <h3 class="text-sm font-black text-slate-800 mb-4 tracking-tight">Verifikasi Keamanan</h3>
          
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <!-- SJ -->
            <div class="floating-group" v-if="form.processType !== 'GBB'">
              <input :value="form.suratJalanNumber" @input="form.suratJalanNumber = $event.target.value.toUpperCase()" type="text" class="floating-input uppercase" placeholder="Nomer Surat Jalan" />
              <label class="floating-label">Surat Jalan (SJ)</label>
            </div>

            <!-- VMS -->
            <div class="floating-group">
              <input :value="form.permitCard" @input="form.permitCard = $event.target.value.toUpperCase()" type="text" class="floating-input uppercase" placeholder="Nomer VMS/Pass" />
              <label class="floating-label">Kartu Ijin / VMS</label>
            </div>
            ...
            <div class="flex space-x-2 md:col-span-2 mt-2">
              <div class="w-1/3 relative">
                ...
                <label class="absolute -top-2 left-3 px-1 text-[10px] font-black tracking-widest uppercase bg-white text-slate-700">Tipe ID</label>
              </div>
              <div class="flex-1 floating-group !pt-0 relative">
                <input :value="form.guestId" @input="form.guestId = $event.target.value.toUpperCase()" type="text" class="floating-input font-mono uppercase" placeholder="Nomer Identitas" />
                <label class="floating-label">Nomer Identitas</label>
              </div>
            </div>

            <!-- Guest Count -->
            <div class="floating-group">
              <input v-model.number="form.guestCount" type="number" min="1" class="floating-input" placeholder="Jumlah Tamu" />
              <label class="floating-label">Jumlah Tamu</label>
            </div>
    ...
    <!-- Action Bar -->
    <div class="px-7 py-5 bg-slate-50/80 border-t border-slate-200 flex justify-between items-center rounded-b-[1.5rem]">
      <button type="button" v-if="currentStep > 1" @click="prevStep" 
        class="btn-secondary px-6 py-2.5 rounded-xl font-black text-xs text-slate-700 hover:text-slate-800 transition-all active:scale-95">
        KEMBALI
      </button>
      <button type="button" v-else @click="$emit('cancel')" 
        class="px-6 py-2.5 rounded-xl border-2 border-slate-200 bg-white font-black text-xs text-slate-700 hover:text-slate-800 hover:border-slate-300 hover:bg-slate-50 transition-all active:scale-95">
        BATAL
      </button>

      <button type="button" v-if="currentStep < 3" @click="nextStep" :disabled="!canProceed"
        class="btn-primary px-8 py-2.5 text-xs transition-all flex items-center" :class="!canProceed ? 'opacity-50 cursor-not-allowed !shadow-none' : ''">
        <span class="font-black">LANJUT</span>
        <span class="material-icons text-[16px] ml-1">arrow_forward</span>
      </button>
      <button type="button" v-if="currentStep === 3" @click="submitForm" :disabled="!canSubmit"
        class="px-8 py-2.5 rounded-xl text-white text-xs font-black flex items-center transition-all shadow-[0_4px_14px_rgba(74,139,223,0.4)]" 
        ...
        <span class="material-icons text-[16px] mr-1.5">how_to_reg</span>
        <span>SELESAI REGISTRASI</span>
      </button>
    </div>
  </div>
</template>
...
const steps = [
  { id: 1, label: 'Tujuan', icon: 'flag' },
  { id: 2, label: 'Kendaraan', icon: 'local_shipping' },
  { id: 3, label: 'Keamanan', icon: 'verified_user' }
]
...
const processOptions = [
  { value: 'GBB', img: '/assets/gbb.png', sub: 'Bahan Baku' },
  { value: 'GBJ', img: '/assets/gbj.png', sub: 'Barang Jadi' },
  { value: 'GSP', img: '/assets/gsp.png', sub: 'Gudang Sparepart' }
]
...
const form = reactive({
  ...
  processType: null, driverName: 'External Driver'
})

// Nomor Polisi: UPPERCASE, NO SPACES — langsung mentah tanpa formatting
...
const canProceed = computed(() => {
  ...
  if (currentStep.value === 2) return !!form.vehicleType && !!form.plateNumber && !!form.vendor
  return true
})
