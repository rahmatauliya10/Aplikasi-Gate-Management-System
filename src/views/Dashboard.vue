<template>
  <div class="space-y-8 pb-10">
    ...
    <div class="rounded-2xl shadow-card overflow-hidden" style="background:white;border:1px solid #E8EEF7">
      ...
      <div class="overflow-x-auto relative max-h-[400px] custom-scrollbar w-full">
        <div class="absolute inset-0 pointer-events-none opacity-[0.03]" style="background-image: linear-gradient(#4A8BDF 1px, transparent 1px), linear-gradient(90deg, #4A8BDF 1px, transparent 1px); background-size: 30px 30px;"></div>
        <table class="w-full table-fixed relative z-10 min-w-[900px]" style="border-collapse: separate; border-spacing: 0 12px;">
          <thead class="sticky top-0 z-20">
            <tr>
              <th class="w-[15%] px-4 sm:px-8 py-3 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest bg-white/90 backdrop-blur shadow-[0_1px_0_#F1F5F9]">Plate No</th>
              <th class="w-[20%] px-4 sm:px-8 py-3 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest bg-white/90 backdrop-blur shadow-[0_1px_0_#F1F5F9]">Vendor / Transport</th>
              <th class="w-[12%] px-4 sm:px-8 py-3 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest bg-white/90 backdrop-blur shadow-[0_1px_0_#F1F5F9]">Warehouse</th>
              <th class="w-[18%] px-4 sm:px-8 py-3 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest bg-white/90 backdrop-blur shadow-[0_1px_0_#F1F5F9]">Current Phase</th>
              <th class="w-[13%] px-4 sm:px-8 py-3 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest bg-white/90 backdrop-blur shadow-[0_1px_0_#F1F5F9]">Time Arrived</th>
              <th class="w-[12%] px-4 sm:px-8 py-3 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest bg-white/90 backdrop-blur shadow-[0_1px_0_#F1F5F9]">Elapsed</th>
              <th class="w-[10%] px-4 sm:px-8 py-3 text-right text-[10px] font-black text-slate-500 uppercase tracking-widest bg-white/90 backdrop-blur shadow-[0_1px_0_#F1F5F9]">Action</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="truck in activeTrucks" :key="truck.id" class="transition-all duration-300 hover:-translate-y-1 hover:shadow-md group bg-white shadow-sm" style="border-radius: 1rem;">
              <td class="px-4 sm:px-8 py-4 whitespace-nowrap border-y border-l border-slate-100 rounded-l-2xl">
                <div class="inline-flex items-center justify-center bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-700 shadow-[inset_0_2px_8px_rgba(0,0,0,0.5)]">
                  <span class="text-[11px] sm:text-xs font-black text-[#4A8BDF] font-mono tracking-widest">{{ truck.plateNumber }}</span>
                </div>
              </td>
              <td class="px-4 sm:px-8 py-4 whitespace-nowrap border-y border-slate-100">
                <div class="text-xs sm:text-sm font-black text-slate-800 truncate" :title="truck.vendor">{{ truck.vendor }}</div>
              </td>
              <td class="px-4 sm:px-8 py-4 whitespace-nowrap border-y border-slate-100">
                <span class="px-2 sm:px-2.5 py-1 text-[8px] sm:text-[9px] font-black rounded-lg uppercase tracking-widest border"
              ...
              <td class="px-4 sm:px-8 py-4 whitespace-nowrap border-y border-slate-100">
                <div class="flex items-center">
                  ...
                  <span class="text-[9px] sm:text-[10px] font-black text-slate-600 uppercase tracking-wider">{{ truck?.step ? truck.step.replace('_', ' ') : '-' }}</span>
                </div>
              </td>
              <td class="px-4 sm:px-8 py-4 whitespace-nowrap text-[11px] sm:text-xs font-bold text-slate-500 font-mono border-y border-slate-100">{{ formatTime(truck?.timestamps?.entry) }}</td>
              <td class="px-4 sm:px-8 py-4 whitespace-nowrap border-y border-slate-100">
                <div class="text-[11px] sm:text-xs font-black text-slate-700 font-mono flex items-center">
                  <span class="material-icons text-[12px] sm:text-[14px] mr-1 text-slate-400">schedule</span>
                  {{ getDuration(truck?.timestamps?.entry) }}
                </div>
              </td>
              <td class="px-4 sm:px-8 py-4 whitespace-nowrap text-right border-y border-r border-slate-100 rounded-r-2xl">
                 <button @click="viewDetails(truck)" class="px-3 py-1 sm:px-4 sm:py-1.5 rounded-lg text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all duration-300 bg-white border border-[#4A8BDF]/20 text-[#4A8BDF] hover:bg-[#4A8BDF] hover:text-white hover:shadow-md">
                   Inspect
                 </button>
              </td>
            </tr>
